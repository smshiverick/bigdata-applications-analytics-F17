...
ties internally. Without restricting their depth, decision trees can become 
complex; unpruned trees are prone to overfitting and do not generalize well 
to new data. Next, pre-pruning was applied, setting the maximum depth to 4, 
which means the algorithm split on the four consecutive questions. Limiting 
the depth of the tree decreases overfitting, which results in lower accuracy 
on the training set, but improved performance on the test set. Even with a  
tree depth of 4, the tree can become a bit complex. Figure 5 shows a partial 
view of the decision tree classifier of heroin use; the entire tree was too 
wide to include as a legible Figure (the full tree image is available in the
analytic-classifier notebook `BDA-Analytics-Classifier-Heroin.ipynb` 
...
following the sample numbers represented at the test split for each node. The 
classifier algorithm selected Cocaine Use (aggregated score) as the root node 
of the decision tree. The branch to the left  side of the tree represents 
...
target and a value of 0 meaning that the feature was not used at all. Feature
importance values also always sum to 1. A feature may have a low feature 
importance value because another feature encodes the same information. The 
top two important features for classifying Heroin Use were Cocaine Use and 
Any Prescription Opioid PRL Use, with smaller importance given to Opioid PRL 
